...
class DatasetAll(torch.utils.data.Dataset):
    def __init__(self, config, partition):
        self.data_path = config['ROOT_DATA_PATH']
        self.face_path = config['FACE_PATH']
        ...
        self.partition_image_names = self.read_image_names(self.data_path)
        ...
        self.tf_img = utils.TransformImage(random_crop=config['random_crop'], random_hflip=config['random_hflip'],
                                           random_vflip=config['random_vflip'])

        self.features = None
        self.labels = None
        self.label_index = None

        self.load_and_sort_pkl(self.data_path)
    ...
    def load_and_sort_pkl(self, data_path):
        ...
        for feature_type in self.feature_types:
            ...
            if feature_type in ['face_extractor', 'gocar']:
                features_input = []
                path = os.path.join(self.feature_path, feature_type, 'X_' + self.partition + '.pickle')
                ...
                for name in self.partition_image_names:
                    ...
                    try:
                        ...
                        # print("Missing data {}: {}".format(feature_type, name))
    ...
    def get_real_labels_fromListoLabels(self, label_list):
        ...
        for label_ml in label_list:
            list_.append(index_label[label_ml])
        return list_
    ...
    def read_images_env(self, data_path):
        ...
        for class_ in self.prediction_classes()[0]:
            data_files += glob.glob(os.path.join(self.image_path(data_path, class_), '*.png'),
                                    recursive=True) + glob.glob(
                os.path.join(self.image_path(data_path, class_), '*.PNG'), recursive=True)
    ...
    def read_images_face(self, data_path):
        ...
        data_path = os.path.join(data_path, self.face_path)
        data_files = []
        for class_ in self.prediction_classes()[0]:
            data_files += glob.glob(os.path.join(self.image_path(data_path, class_), '*.png'),
                                    recursive=True) + glob.glob(
        ...
        for feature_type in self.feature_types:
            if 'img' in feature_type:
                if self.features[feature_type][index] is not None:
                    ...
                    input_tensor = torch.Tensor(3, 299, 299).new_full((3, 299, 299), torch.finfo(torch.float32).eps)
